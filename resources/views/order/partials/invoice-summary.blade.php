        ...
        <div class="space-y-3">
            @if(Auth::user()->role === 'tukang')
                ...
                @elseif($order->status === 'pengerjaan')
                    <form action="{{ route('tukang.orders.complete', $order->id) }}" method="POST">
                        @csrf
                        <button type="submit" class="w-full bg-green-500 hover:bg-green-600 text-white py-4 rounded-2xl font-bold shadow-lg shadow-green-500/25 transition transform hover:-translate-y-1 block text-center uppercase tracking-widest text-xs">
